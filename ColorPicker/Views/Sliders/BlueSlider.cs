...
namespace ColorPicker {
    public class BlueSlider : GraphicalSlider {
        ...
        protected override void SetValue(double value, bool user_operation, bool internal_only = false) {
            SetSelectedColor(new RGB(SelectedColor.R, SelectedColor.G, value), user_operation, internal_only);
        ...
        protected void SetSelectedColor(RGB color, bool user_operation, bool internal_only = false) {
            if (prev_color.R != color.R || prev_color.G != color.G) {
                if (prev_color.B != color.B) {
                    prev_color = color;
                    base.SetValue(color.B, user_operation);
                }
                else {
                    prev_color = color;
                }
            ...
            unsafe {
                fixed (byte* c = buf) {
                    for (int x = 0, i = 0; x < width; x++, i += 4) {
                        byte b = (byte)double.Clamp(x * scale, 0, 255);
