...
namespace ColorPicker {
    public class SaturationSlider : GraphicalSlider {
        ...
        protected override void SetValue(double value, bool user_operation, bool internal_only = false) {
            SetSelectedColor(new HSV(SelectedColor.H, value, SelectedColor.V), user_operation, internal_only);
        ...
        protected void SetSelectedColor(HSV color, bool user_operation, bool internal_only = false) {
            if (current_color.H != color.H || current_color.V != color.V) {
                if (current_color.S != color.S) {
                    current_color = color;
                    base.SetValue(color.S, user_operation, internal_only);
                }
                else {
                    current_color = color;
                }
            ...
            else if (current_color.S != color.S) {
                current_color = color;
